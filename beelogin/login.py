...
def root(request: Request, session: SessionData = Depends(get_session), settings: config.Settings = Depends(config.get_settings)):
    ...
    return templates.TemplateResponse(
        request=request, name="index.html", context={
            ...
            "g_redirect_uri": settings.g_redirect_uri,
...
def g_callback(
    ...
    session.nonce = None # <--- CLEAR THE SESSION NONCE EARLY
    ...
    if not id_token:
        raise HTTPException(
            status_code=400, 
            detail="ID token missing from Google response."
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, 
            detail="Failed to decode Google ID token."
        )
    ...
    if actual_nonce != expected_nonce: # <--- COMPARE STORED NONCE WITH TOKEN NONCE
        raise HTTPException(401, detail="Nonce mismatch in ID token. Possible replay attack.")
    ...
    if email:
        ...
        raise HTMLResponse(status_code=500, detail="Unexpected user info.")
